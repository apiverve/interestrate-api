...
class InterestrateValidationException implements Exception {
  final List<String> errors;

  InterestrateValidationException(this.errors);

  @override
  String toString() => 'InterestrateValidationException: ${errors.join("; ")}';
}
...
/// Interest Rate API client.
///
/// For more information, visit: https://apiverve.com/marketplace/interestrate?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [country] (required) - 2-letter country code [minLength: 2, maxLength: 2]
class InterestrateClient {
  ...
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw InterestrateValidationException(errors);
    }
  }

  /// Execute a request to the Interest Rate API.
  ///
  /// Parameters are validated before sending the request.
  Future<InterestrateResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw InterestrateException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return InterestrateResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw InterestrateException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw InterestrateException('Resource not found');
      } else {
        throw InterestrateException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is InterestrateException) rethrow;
      throw InterestrateException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Interest Rate API client.
class InterestrateException implements Exception {
  final String message;

  InterestrateException(this.message);

  @override
  String toString() => 'InterestrateException: $message';
}
